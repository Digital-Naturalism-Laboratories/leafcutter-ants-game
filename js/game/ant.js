...
class Ant {
    constructor(leaf) {
        ...
        this.cutClicks = 3;
        this.currentCutClicks = 0;
        this.cutDuration = 120 / 3;
        this.cutTimer = 0;
        ...
        this.timedJawSpeedFactorStep = 0;//0.005; //Timing change factor removed!
    ...
    drawJawControls() {
        if (this.rotationMode) {
            ...
            drawCircle(renderer, this.isCuttingJawLed ? this.cuttingJawControlPos : this.leadingJawControlPos,
                this.timedJawRadius, false, this.jawSpeedPenalty <= 0 ? rgba(0, 255, 0, 0.75) : "#99999955", (this.timedJawRadius - this.timedJawMinRadius));
        ...
        if (this.rotationMode) {
            if (isPointInCircle(vec2(touchPos[0].x - canvasStartX, touchPos[0].y - canvasStartY), this.cuttingJawControlPos, this.timedJawMinRadius)) {
                if (this.isCuttingJawLed) {
                    ...
                    this.currentCutClicks--;
                    if (this.currentCutClicks <= 0) {
                        this.timedJawCutTimeBonus = 0;
                        this.isCuttingJawLed = false;
                    }
                ...
                if (!this.isCuttingJawLed) {
                    ...
                    this.currentCutClicks = this.cutClicks;
                }
                //Jaw Penalty Removed
                /*else
                {
